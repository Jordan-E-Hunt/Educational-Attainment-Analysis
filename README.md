...
* `data/`: Sanitized Census and NCES datasets.
* `notebooks/`: Exploratory Data Analysis (EDA) and visualization logic.
* `presentations/`: Final PDF report and slide decks.
* `Master_Script.ipynb`: The primary pipeline for ingestion, sanitization, and deployment.
...
# Implementation of the Defensive Sanitization Layer
def secure_ingestion(file_path):
    # Neutralizes binary infection mechanisms/superscripts
    df = pd.read_excel(file_path, engine='openpyxl')
    return df.applymap(lambda x: re.sub(r'[^\x20-\x7E]', '', str(x)).strip() 
